...
public class IAfacilesau extends JFrame implements ActionListener {
	...
	public int[][] tableaujeu() {
		...
		try {
			InputStream ips=new FileInputStream("sauvegardes/partie13carrefa/sauvegarde.txt"); 
	...
	public int[][] tableaujoueur() {
		...
		try {
			InputStream ips=new FileInputStream("sauvegardes/partie13carrefa/sauvegardejoueur.txt"); 
			InputStreamReader ipsr=new InputStreamReader(ips);
			BufferedReader br=new BufferedReader(ipsr);
			String chiffre;
			int i = 0;
	...
	public IAfacilesau(){
	  ...
	  if (exist>0) {
		  ...
		  JLabel aucun = new JLabel("Pas de partie sauvegardée");
		  aucun.setFont(new Font("Tahoma", Font.BOLD, 25));
		  aucun.setForeground(Color.black);
		  this.getContentPane().add(aucun, BorderLayout.CENTER);
	  }
